...
class Bi_BaseBackbone(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.stem = self.build_stem_t_layer()
        self.t_layers = ['stem']
        self.idam = ['']
        ...
        for idx, setting in enumerate(arch_setting):
            idam = []
            idam += self.build_stage_idam(idx, setting)
            self.add_module(f'idam{idx + 1}', nn.Sequential(*idam))
            self.idam.append(f'idam{idx + 1}')

        for idx, setting in enumerate(arch_setting):
            t_stage = []
            t_stage += self.build_stage_t_layer(idx, setting)
            if plugins is not None:
                t_stage += self.make_stage_plugins(plugins, idx, setting)
            #########################################################
            self.add_module(f'stage{idx + 1}', nn.Sequential(*t_stage))
            self.t_layers.append(f'stage{idx + 1}')
    ...
    @abstractmethod
    def build_stem_t_layer(self):
        """Build a stem layer."""
        pass

    @abstractmethod
    def build_stem_idam(self):
        """Build a stage idam."""
        pass

    @abstractmethod
    def build_stage_idam(self):
        """Build a stage idam."""
        pass
    ...
    @abstractmethod
    def build_stage_t_layer(self, stage_idx: int, setting: list):
        """Build a stage layer.
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        # TODO concat rgb and t in the feature stage
        for i, layer_name in enumerate(self.layers):
            layer = getattr(self, self.layers[i])
            t_layer = getattr(self,  self.t_layers[i])
            rgb = layer(rgb)
            t = t_layer(t)
            if i in self.out_indices:
                layer_idam = getattr(self,self.idam[i])
                rgb, t, fuse = layer_idam([rgb, t])[0], layer_idam([rgb, t])[1],layer_idam([rgb, t])[2]

                # out = torch.cat([rgb,t,fuse],dim=1)
                # outs.append(out)
